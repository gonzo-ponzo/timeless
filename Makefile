...
flower:
	cd backend && celery -A tasks.tasks flower --logfile=./flower.log --loglevel=INFO --basic-auth=gonzo:fuckingflower

worker:
	cd backend && celery -A tasks.celery worker -f worker.log --loglevel=INFO